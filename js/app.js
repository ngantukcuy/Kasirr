...
const KasirkuDB = {
  ...
  Transactions: {
    ...
    async updateItemQty(transactionId, itemId, newQty, productId, oldQty, invoiceNumber) {
      // 1. Get current item data
      const { data: item, error: iErr } = await _sb.from('transaction_items')
        .select('selling_price').eq('id', itemId).single();
      if (iErr) throw iErr;

      const newSubtotal = newQty * item.selling_price;

      // 2. Update transaction_item
      const { error: itemErr } = await _sb.from('transaction_items')
        .update({ quantity: newQty, subtotal: newSubtotal }).eq('id', itemId);
      if (itemErr) throw itemErr;

      // 3. Adjust stock by diff only (bukan qty penuh!)
      const diff = newQty - oldQty;
      if (productId && diff !== 0) {
        const { data: prod } = await _sb.from('products').select('stock').eq('id', productId).single();
        if (prod) {
          const newStock = prod.stock - diff;
          await _sb.from('products').update({ stock: newStock }).eq('id', productId);
          await _sb.from('stock_movements').insert({
            ...
            notes: `Edit qty transaksi ${invoiceNumber}: ${oldQty} → ${newQty}`
          });
        }
      }

      // 4. Recalculate transaction total
      const { data: allItems } = await _sb.from('transaction_items')
        .select('subtotal').eq('transaction_id', transactionId);
      const newSubtotalTx = (allItems || []).reduce((s, i) => s + i.subtotal, 0);

      const { data: tx } = await _sb.from('transactions')
        .select('discount_amount').eq('id', transactionId).single();
      const newTotal = newSubtotalTx - (tx?.discount_amount || 0);

      await _sb.from('transactions')
        .update({ subtotal: newSubtotalTx, total_amount: newTotal }).eq('id', transactionId);
    }
  },
